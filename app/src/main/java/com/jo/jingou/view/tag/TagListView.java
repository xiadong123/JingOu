 ...
 *
 */
package com.jo.jingou.view.tag;
...
//        }
    ...
    public Tag getHasCheckedTag() {
        for (Tag tag : mTags) {
            if (tag.isChecked()) return tag;
        }
        return null;
    }

    public List<Tag> getHasCheckedTags() {
        List<Tag> tagList = new ArrayList<>();
        for (Tag tag : mTags) {
            if (tag.isChecked()) tagList.add(tag);
        }
        return tagList;
    }
