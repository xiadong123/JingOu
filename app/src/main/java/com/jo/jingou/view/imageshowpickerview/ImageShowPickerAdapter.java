...
public class ImageShowPickerAdapter extends RecyclerView.Adapter<ImageShowPickerAdapter.ViewHolder> implements
        ImageShowPickerPicListener {
    ...
    public void setShowAddPic(boolean isShowAddPic) {
        this.isShowAddPic = isShowAddPic;
    }

    public ImageShowPickerAdapter(int mMaxNum, Context context, List<ImageShowPickerBean> list, ImageLoaderInterface
            imageLoaderInterface, ImageShowPickerListener pickerListener) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        frameLayout.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout
        ...
        if (list.size() == 0 || list.size() == position) {
            ...
            if (!isShowAddPic) {
                ...
                FrameLayout.LayoutParams pic_params = new FrameLayout.LayoutParams(iconHeight,
                        iconHeight);
                pic_params.setMargins(0, 10, 10, 10);
                holder.iv_pic.setLayoutParams(pic_params);
            }
        } else {
            FrameLayout.LayoutParams pic_params = new FrameLayout.LayoutParams(iconHeight,
                    iconHeight);
            ...
            holder.iv_pic.setLayoutParams(pic_params);
            if (null == list.get(position).getImageShowPickerUrl() || "".equals(list.get(position)
                    .getImageShowPickerUrl())) {
                imageLoaderInterface.displayImage(context, list.get(position).getImageShowPickerDelRes(), holder
                        .iv_pic);
            } else {
    ...
    @Override
    public void onPicClickListener(int position, View view) {
        if (position == list.size()) {
            ...
            if (pickerListener != null) {
                ...
                pickerListener.picOnClickListener(list, position, mMaxNum > list.size() ? mMaxNum - list.size() - 1 :
                        (list.get(mMaxNum - 1) == null ? 1 : 0), view);
            }
        }
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public ViewHolder(View view, ImageLoaderInterface imageLoaderInterface, ImageShowPickerPicListener
                picOnClickListener) {
            super(view);
        ...
        public void onClick(View v) {
            ...
            if (i == R.id.iv_image_show_picker_pic) {
                picOnClickListener.onPicClickListener(getLayoutPosition(), v);
